					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<select name="kasi" id="kasi" class="form-control" required>
										<option value="" selected>======== Pilih Atasan ========</option>
										<?php
										$mySqli = "SELECT employee.* FROM employee WHERE (hak_akses='kasi' AND active='Aktif' AND proyek1='$proyek') OR (proyek2='$proyek' AND hak_akses='kasi' AND active='Aktif') ORDER BY nama_emp";
										$myQuery = mysqli_query($conn, $mySqli);
										$dataKasi = $result['npp'];
										while ($kasiData = mysqli_fetch_array($myQuery)) {
											if ($kasiData['npp'] == $dataKasi) {
												$cek = " selected";
											} else {
												$cek = "";
											}
											echo "<option value='$kasiData[npp]' $cek>" . $kasiData['nama_emp'] . "</option>";
										}
										?>
									</select>
								</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<select name="pengganti" id="pengganti" class="form-control" required>
										...
										<?php
										$mySql = "SELECT employee.* FROM employee WHERE (hak_akses='pegawai' AND active='Aktif' AND npp NOT IN ($npp) AND proyek1='$proyek') OR (proyek2='$proyek' AND hak_akses='pegawai' AND active='Aktif' AND npp NOT IN ($npp)) ORDER BY nama_emp";
