...
$kas = mysqli_query($conn, "SELECT employee.* FROM employee WHERE npp='$kasi'");
$k = mysqli_fetch_array($kas);
$leader = $k['email'];
...
if ($hak == "pegawai") {
	if ($aksi == "2") {
		...
		$num = 1;
		$sql = "UPDATE cuti SET stt_cuti='" . $stt . "' WHERE no_cuti='" . $no . "'";
		$ress = mysqli_query($conn, $sql);
	...
	if ($aksi == "2") {
		...
		$num = 1;
		$sql = "UPDATE cuti SET stt_cuti='" . $stt . "', dmp='" . $dmp . "' WHERE no_cuti='" . $no . "'";
		$ress = mysqli_query($conn, $sql);
