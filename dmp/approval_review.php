						...
						<div class="panel-body">
							<div class="form-group">
								...
								<div class="col-sm-4">
									...
									<?php
									$Sqli = "SELECT employee.* FROM employee WHERE employee.npp=cuti.npp";
									$Query = mysqli_query($conn, $Sql);
									$p = mysqli_fetch_array($Query);
									?>
									<input type="hidden" name="hak" class="form-control" value="<?php echo $p['hak_akses']; ?>">
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<?php
								$hrd = "SELECT employee.* FROM employee WHERE hak_akses='admin' AND active='Aktif'";
								$de = mysqli_query($conn, $hrd);
								while ($hrdData = mysqli_fetch_array($de)) {
								?>
									<input type="hidden" name="hrd" class="form-control" value="<?php echo $hrdData['npp'] ?>" required>
								<?php
								}

								$mp = "SELECT employee.* FROM employee WHERE hak_akses='MP' AND active='Aktif'";
								$de = mysqli_query($conn, $mp);
								while ($mpData = mysqli_fetch_array($de)) {
								?>
									<input type="hidden" name="mp" class="form-control" value="<?php echo $mpData['npp'] ?>" required>
								<?php
								}
								?>
							</div>
